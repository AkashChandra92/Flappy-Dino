...
const bird = {
  ...
  update: function() {
    ...
    if (state.current == state.getReady) {
      ...
      //  IF THE SPEED IS GREATER THAN THE JUMP MEANS THAT BIRD IS FALLING DOWN
...
const getReady = {
  ...
  draw: function() {
    // ctx.fillText("Flappy Dino", cvs.width/2,50)
...
const pipes = {
  ...
  update: function() {
    ...
    for (let i = 0; i < this.position.length; i++) {
      ...
      let bottompPipeYPos = p.y + this.h + this.gap;
      ...
      if (
        bird.x + bird.radius > p.x &&
        bird.x - bird.radius < p.x + this.w &&
        bird.y + bird.radius > bottompPipeYPos &&
        bird.y - bird.radius < bottompPipeYPos + this.h
      ) {
        state.current = state.over;
        HIT.play();
      }
